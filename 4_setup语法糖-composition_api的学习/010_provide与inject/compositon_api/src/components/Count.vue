...
export default {
  ...
  setup(props, context) {
    ...
    /* computed计算属性在setup当中的使用 */
    let sum = computed(() => num.value + 100);

    const user = inject("user", "默认");

    const updateUser = inject("updateUser", "默认");
    let up = () => {
      updateUser("我会用了");
    };
    ...
    return { num, add, sub, attrs, defaults, sum, user, updateUser, up }; //总结：返回出去的这些功能 能在expose里面能找到的
  },
